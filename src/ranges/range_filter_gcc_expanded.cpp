...
template <std::ranges::forward_range _Range> struct _CachedPosition<_Range> {
private:
  std::ranges::iterator_t<_Range> _M_iter{};

public:
  constexpr bool _M_has_value() const {
    return _M_iter != std::ranges::iterator_t<_Range>{};
  }
  ...
  constexpr void _M_set(const _Range &,
                        const std::ranges::iterator_t<_Range> &__it) {
    __glibcxx_assert(!_M_has_value());
    _M_iter = __it;
  }
};

template <std::ranges::random_access_range _Range>
requires(sizeof(std::ranges::range_difference_t<_Range>) <=
         sizeof(std::ranges::iterator_t<_Range>)) struct _CachedPosition<_Range> {
private:
  std::ranges::range_difference_t<_Range> _M_offset = -1;

public:
  constexpr bool _M_has_value() const { return _M_offset >= 0; }

  constexpr std::ranges::iterator_t<_Range> _M_get(_Range &__r) const {
    __glibcxx_assert(_M_has_value());
    return std::ranges::begin(__r) + _M_offset;
  }

  constexpr void _M_set(_Range &__r, const std::ranges::iterator_t<_Range> &__it) {
    __glibcxx_assert(!_M_has_value());
    _M_offset = __it - std::ranges::begin(__r);
  }
};
...
namespace custom_views::__adaptor {
...
struct _Filter : custom_views::__adaptor::_RangeAdaptor<_Filter> {
