// Original version by Miro Knejp 
...
auto run_with_computed_goto(bytecode const *instructions) {
  ...
  constexpr void *labels[] = {
      [bytecode::add1] = &&add1_label, [bytecode::add2] = &&add2_label,
      [bytecode::add3] = &&add3_label, [bytecode::add5] = &&add5_label,
      [bytecode::add7] = &&add7_label, [bytecode::sub1] = &&sub1_label,
      [bytecode::sub2] = &&sub2_label, [bytecode::sub3] = &&sub3_label,
      [bytecode::sub5] = &&sub5_label, [bytecode::sub7] = &&sub7_label,
      [bytecode::mul2] = &&mul2_label, [bytecode::mul3] = &&mul3_label,
      [bytecode::mul5] = &&mul5_label, [bytecode::div2] = &&div2_label,
      [bytecode::div3] = &&div3_label, [bytecode::div5] = &&div5_label,
...
auto run_with_tail_recursion(bytecode const *instructions) {
  static double (*const funs[17])(double, bytecode const *) = {
      [bytecode::add1] =
          [](auto value, auto *inst) { return funs[*inst](value + 1.0, inst + 1); },
      [bytecode::add2] =
          [](auto value, auto *inst) { return funs[*inst](value + 2.0, inst + 1); },
      [bytecode::add3] =
          [](auto value, auto *inst) { return funs[*inst](value + 3.0, inst + 1); },
      [bytecode::add5] =
          [](auto value, auto *inst) { return funs[*inst](value + 5.0, inst + 1); },
      [bytecode::add7] =
          [](auto value, auto *inst) { return funs[*inst](value + 7.0, inst + 1); },
      [bytecode::sub1] =
          [](auto value, auto *inst) { return funs[*inst](value - 1.0, inst + 1); },
      [bytecode::sub2] =
          [](auto value, auto *inst) { return funs[*inst](value - 2.0, inst + 1); },
...
auto create_instructions(std::size_t count,
  ...
  auto dist =
      std::uniform_int_distribution<int>(0, static_cast<int>(bytecode::halt) - 1);
  std::generate_n(std::back_inserter(instructions), count - 1,
...
static auto BM_switch_loop(benchmark::State &state, int) {
  auto const instructions =
      create_instructions(static_cast<std::size_t>(state.range(0)));
